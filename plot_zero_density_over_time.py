...
def plot_zero_percentage():
	...
	plt.plot(df_trades["day"], df_trades["zero_percentage"], color = "green", label = "Trades")
	plt.plot(df_messages["day"], df_messages["zero_percentage"], color = "blue", label = "Messages")
	plt.xlim(0.8, 30.5)
	plt.xticks(range(1, 31), fontsize = 12)
	plt.yticks(fontsize = 12)
	plt.xlabel("Day", fontsize = 15)
	plt.ylabel("Percentage", fontsize = 15)
	plt.title("Percentage of relevant alliances with density equal to zero")
	plt.legend()
	plt.tight_layout()
